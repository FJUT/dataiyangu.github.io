...
categories:
  - 数据库
  - 数据库基础知识
date: 2018-11-09 13:17:00
---
前两天经历了武汉一行腾讯面试，数据库索引是一个面试热点，在此搜集相关资料，以备学习之用。

下面是一位牛人写得关于数据库索引的精品之作，因为很好，不敢修饰，转载至此与博友共享。
...
![upload successful](/images/my_blog_115.png)
...
![upload successful](/images/my_blog_116.png)
...
![upload successful](/images/my_blog_117.png)
...
![upload successful](/images/my_blog_118.png)
图4
...
![upload successful](/images/my_blog_119.png)
...
![upload successful](/images/my_blog_120.png)
...
![upload successful](/images/my_blog_121.png)
...
![upload successful](/images/my_blog_122.png)
...
![upload successful](/images/my_blog_123.png)
...
![upload successful](/images/my_blog_124.png)
...
![upload successful](/images/my_blog_125.png)
...
为了讨论索引策略，需要一个数据量不算小的数据库作为示例。本文选用MySQL官方文档中提供的示例数据库之一：[employees](https://launchpad.net/test-db/employees-db-1/1.0.6)。这个数据库关系复杂度适中，且数据量较大。下图是这个数据库的E-R关系图（引用自MySQL官方手册）：

![upload successful](/images/my_blog_126.png)
...
从结果中可以到titles表的主索引为<emp\_no, title, from\_date>，还有一个辅助索引<emp_no>。为了避免多个索引使事情变复杂（MySQL的SQL优化器在多索引时行为比较复杂），这里我们将辅助索引drop掉：
...
当查询条件精确匹配索引的左边连续一个或几个列时，如<emp\_no>或<emp\_no, title>，所以可以被用到，但是只能用到一部分，即条件所组成的最左前缀。上面的查询从分析结果看用到了PRIMARY索引，但是key_len为4，说明只用到了索引的第一列前缀。
...
如果频繁按名字搜索员工，这样显然效率很低，因此我们可以考虑建索引。有两种选择，建<first\_name>或<first\_name, last_name>，看下两个索引的选择性：
...
<first\_name>显然选择性太低，<first\_name, last\_name>选择性很好，但是first\_name和last\_name加起来长度为30，有没有兼顾长度和选择性的办法？可以考虑用first\_name和last\_name的前几个字符建立索引，例如<first\_name, left(last_name, 3)>，看看其选择性：
...
![upload successful](/images/my_blog_127.png)
...
![upload successful](/images/my_blog_128.png)
